...
$edit = "select DISTINCT t3.cafeName,t3.numTable,t3.numChair,numIn,numOut from cafeInfo as t3 join (select t1.cafeID, t1.uid, t1.numIn, t1.numOut from ESPtable as t1,(select cafeID, max(uid) as max_uid from ESPtable group by cafeID ) as t2 where t1.uid=t2.max_uid and t1.cafeID=t2.cafeID) as t4 on t3.cafeID=t4.cafeID where t3.name LIKE '%$n%'";
...
<body>
	<nav class="row">
		...
		<ul>
			<li class="col-1 col-s-1"><a href="/index.php">첫화면으로</a></li>
		</ul>
	</nav>
	<section>
		<table class="center">
